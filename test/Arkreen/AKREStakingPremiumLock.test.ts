...
describe("StakingRewardsLock test", ()=> {
    ...
    let stakingRewards:           StakingRewardsLock
    ...
    async function deployFixture() {
        ...
        const stakingRewardsFactory = await ethers.getContractFactory("StakingRewardsLock")
        const stakingRewards = await upgrades.deployProxy(stakingRewardsFactory, 
                                        [arkreenToken.address, artToken.address, arkreenMiner.address, deployer.address]) as StakingRewardsLock
    ...
    describe('StakingRewardsLock test', () => {
      ...
      it("StakingRewardsLock basics", async function () {
      ...
      it("StakingRewardsLock Lock basics", async function () {
        ...
        await stakingRewards.setLockParameter(lastBlock.timestamp + startTime, duration)

        const lockDuration = await stakingRewards.lockDuration()
        const switchTimestamp = await stakingRewards.switchTimestamp()

        console.log("SSSSSSSSSSS", switchTimestamp, lockDuration)
        ...
        const amountAKREa = await arkreenToken.balanceOf(user1.address)
        await userUnstake(1, stake1)

        const lockIndexValue = await stakingRewards.lockIndex(user1.address)
        const lockInfoKey = BigNumber.from(user1.address).shl(96).add(0)
        const lockInfo = await stakingRewards.lockInfo(lockInfoKey)

        console.log('AAAAAAAAAAAAAA', lockIndexValue, lockInfoKey, lockInfo)

        expect(await arkreenToken.balanceOf(user1.address)).to.eq(amountAKREa)

        ///////////// Unlock //////////////////

        const unlockAmount = await stakingRewards.unstakeStatus(user1.address)
        console.log('BBBBBBBBBBBBBBBBBBB', unlockAmount, lastBlockN.timestamp, lastBlockN)

        await expect(stakingRewards.connect(user1).claimStake()).to.be.revertedWith("Not ready")
...

        {
          ...
          await stakingRewards.connect(user1).claimStake()
        }

        expect(await arkreenToken.balanceOf(user1.address)).to.eq(amountAKREa.add(stake1))
      ...
      it("StakingRewardsLock: Miner onboard", async function () {
      ...
      it("StakingRewardsLock before stake deposit", async function () {
      ...
      it("StakingRewardsLock deposit with permit", async function () {
      ...
      it("StakingRewardsLock stake 3 rounds", async function () {
      ...
      it("StakingRewardsLock stake 3 rounds with lock activated", async function () {
